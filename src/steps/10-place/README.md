...
Next we will place blocks. We'll spawn them as entities on a grid, place them with a right click on the ground or on another block, and make them solid so the player can climb them.
...
Create `block/actions.ts`. `spawnBlockAt` snaps any position to a cell and refuses a cell that is taken or has a body in it. `placeBlock` works out which cell a click means. You can use both as provided and focus on what they take in.
...
The action does not know about clicks. It takes a surface and where it was hit, which any view can supply. The rules for what makes a valid placement live here, in one place.
...
Fiber raycasts pointer events into the scene, so `event.point` is where the ray hit and `event.face.normal` is which way that face points. `preventDefault` keeps the browser's context menu closed.
...
Append the collision code. You can use `resolveOverlap` as provided and focus on the system below it.
...
In `frameloop.tsx`, run it after the floor, so a block can re-ground a player the floor just let go of.
...
Run `pnpm dev` and open [your practice game](http://localhost:5173/). Right click the ground to place a block, then right click the top of a block to stack one. Walk into a stack and jump onto it. Try to place a block where you stand: nothing happens.
