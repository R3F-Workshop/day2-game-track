...
Next we will add pigs that wander around. A pig has the same controller as the player, and the only difference is what writes its `Input`.
...
Create `pig/actions.ts`. Compare `spawnPig` with `spawnPlayer`: the same traits with different numbers, plus `Wander` in place of a keyboard.
...
From here on the pig is a character like any other. The controller moves it, physics grounds it, the state machine names what it is doing, and blocks stop it.
...
Run `pnpm dev` and open [your practice game](http://localhost:5173/). Two pigs amble around and stop to rest. Press R for more. Build a wall and watch a pig walk into it: it pushes until its timer runs out, then picks another way.

You have a game. From here the paths branch: a first-person camera, more block kinds, terrain generation, riding the pigs. Each one starts with a trait.
